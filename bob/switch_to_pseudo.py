...
def writeTextToConsole(svgfile, name = "/Users/andrew/PycharmProjects/Experiments/psudocode/pythonfixed.py"):
    """
...
def indentationFinder(svgfile, linesToAvoid):
    ...
    """
    searchFor = [["if", "ENDIF", [], [], False], ["def", "Function ends here", [], [] , True ], ["class", "ENDCLASS", [], [] , True ], ["while", "ENDWHILE", [], [] , False ], ["for", "For loop ends here", [], [] , False]]
    #print("CLEAR")
...
def pseudocode():
    ...
    for x in range(0, 10):
        svgfile = wordReplacer(svgfile, linesToAvoid, [["def", "Function with name"], ["while true", "while true"], ["print", "Print to console"], ["self.", " "], ["return", "Return the value"], ["else:", "Otherwise:"], ["==", "is equal to"], ["if", "If"], ["or", "OR"], ["and", "AND"],["and", "AND"], ["class", "CLASS"], ["+=", "is increased by"], ["-=", "is decreased by"], ["*=", "is multiplyed by"], ["/=", "is divided by"], ["<=", "is less than or equal to"], [">=", "is greater than or equal to"], [">", "is greater than"], ["<","is less than"], ["for", "For the variable"], ["while", "While the value is"], ["else", "Otherwise"], ["elif", "Otherwise, if"], ["rasie", "throws a"], ["input", "Ask the user to"], ["!=", "not equal to"], ["break", "exit from loop"] ])
    for x in range(0, 5):
        svgfile = wordReplacer(svgfile, linesToAvoid, [["self.", " "],], False)
    for x in range(0, 10):
        svgfile = wordReplacer(svgfile, linesToAvoid, [["=", "has a value of"], ["~~~", "ELSEIF"]])
    for x in range(0, 10):
    ...
    writeTextToConsole(svgfile, filename)                            #Finally write the new text file!
